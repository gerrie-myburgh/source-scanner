...
export class ScannerSettingsTab extends PluginSettingTab {
    ...
    textScanner(): void {
                    ...
                    .onClick((cb: MouseEvent) => {
                        dialog.showOpenDialog({ properties: ['openDirectory'] })
                            .then(async (result: { canceled: any; filePaths: string[]; }) => {
                                ...
                                appPathSetting.setDesc(`Application workspace: ${this.plugin.settings.dir}`)
                                await this.plugin.saveSettings();
                            }).catch((err: any) => {
                                console.log(err)
                            });
        ...
        const workFolderInVault = new Setting(containerEl)
            .setName("Work Folder")
            .setDesc("The Work folder in the vault")
            .addText((text) =>
                text
                    .setPlaceholder("Enter your work folder string")
